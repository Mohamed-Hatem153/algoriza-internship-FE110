<template>
  <div class="container text-footer-color mt-20">
    <div class="text-center">
      <h2 class="text-3xl font-semibold">
        Explore the world with My Dream place
      </h2>
      <p class="text-button-color mt-2 mb-20">
        Discover new places and experiences
      </p>
    </div>
    <div class="flex justify-between">
      <div class="flex flex-col justify-start">
        <h3 class="text-lg flex font-medium tracking-wide">
          <span class="mr-1"
            ><svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
            >
              <path
                d="M3.414 13.7782C2.5 14.0002 2 15.1922 2 15.1922C2 15.1922 5.398 15.6262 6.949 17.3132C8.5 19.0002 9.071 22.2632 9.071 22.2632C9.071 22.2632 10.47 22.6982 10.485 20.8492C10.5 19.0002 9.778 17.3132 9.778 17.3132L13.091 14.0002L16.701 21.7042C16.701 21.7042 18.08 22.2302 18.04 20.3652C18 18.5002 16.85 10.2422 16.85 10.2422L19.678 7.41319C19.869 7.22869 20.0214 7.008 20.1262 6.76399C20.231 6.51999 20.2862 6.25755 20.2885 5.99199C20.2908 5.72643 20.2402 5.46307 20.1396 5.21728C20.0391 4.97149 19.8906 4.74818 19.7028 4.5604C19.515 4.37261 19.2917 4.2241 19.0459 4.12354C18.8001 4.02298 18.5368 3.97238 18.2712 3.97468C18.0056 3.97699 17.7432 4.03217 17.4992 4.13698C17.2552 4.2418 17.0345 4.39417 16.85 4.58519L13.947 7.48819C13.947 7.48819 6.148 6.09419 3.824 6.29719C1.5 6.50018 2.559 7.56319 2.559 7.56319L10.203 11.2332L6.95 14.4862C6.95 14.4862 4.328 13.5562 3.414 13.7782Z"
                fill="#2F80ED"
              /></svg></span
          >My Dream Place
        </h3>
        <p class="text-sm">Your next goto companion for travel</p>
      </div>
      <div class="flex flex-col text-sm">
        <h3 class="text-base mb-5 font-medium">Company</h3>
        <p class="mb-3">Jobs</p>
        <p class="mb-3">About</p>
        <p class="mb-3">Newsroom</p>
        <p class="mb-3">Advertising</p>
        <p>Contact us</p>
      </div>
      <div class="flex flex-col text-sm">
        <h3 class="text-base mb-5 font-medium">Explore</h3>
        <p class="mb-3">Australia</p>
        <p class="mb-3">New Zealand</p>
        <p class="mb-3">United States America (USA)</p>
        <p class="mb-3">Greece</p>
        <p class="mb-3">Maldives us</p>
        <p class="mb-3">Singapore</p>
        <p class="text-button-color">See more</p>
      </div>
      <div class="flex flex-col text-sm">
        <h3 class="text-base mb-5 font-medium">Terms and Policies</h3>
        <p class="mb-3">Privacy Policy</p>
        <p class="mb-3">Terms of use</p>
        <p class="mb-3">Acessibility</p>
        <p>Reward system policy</p>
      </div>
      <div class="flex flex-col text-sm">
        <h3 class="text-base mb-5 font-medium">Help</h3>
        <p class="mb-3">Support</p>
        <p class="mb-3">Cancel your bookings</p>
        <p class="mb-3">Use Coupon</p>
        <p class="mb-3">Refund Policies</p>
        <p>International Travel Documents</p>
      </div>
    </div>
  </div>
  <div
    class="mt-14 bg-sub-footer py-3 pr-52 text-end text-place-holder text-sm"
  >
    <p>&copy; my Dream Place 2022</p>
  </div>
</template>

<script setup></script>

<style></style>
